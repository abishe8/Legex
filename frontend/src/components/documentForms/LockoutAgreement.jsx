...
function LockoutAgreement() {
   ...
   function validateSaleDeedFormData(formData) {
      const labels = {
         ...
         "advance-amount-paid-in-words": "Advance Amount (in words)",
         "remaining-balance-amount": "Remaining Balance Amount",
         "remaining-balance-amount-in-words": "Remaining Balance (in words)",
      ...
      Object.entries(formData).forEach(([key, raw]) => {
         ...
         switch (key) {
            ...
            case "agreement-year":
               if (!/^\d{4}$/.test(val)) {
                  errors[key] = "Enter a valid agreement year";
               }
               break;
            ...
            // no extra checks for plot-area, plot-address, survey numbers, etc.
   ...
   const handleSubmit = async (e) => {
      ...
      const { isValid, errors } = validateSaleDeedFormData(formData);
      if (!isValid) {
      ...
      try {
         ...
         navigate("/display-summary", {
            state: {
               ...
               formData,
               documentType: "lockout-agreement",
            },
         });
      } catch (error) {
