...
function VehicleSaleAgreement() {
   ...
   function validateFormData(formData) {
      // user-friendly labels for each key
      const labels = {
         ...
         "vehicle-name-model": "Vehicle Name & Model",
         "buyer-name": "Buyer’s Name",
         "buyers-fathers-name": "Buyer’s Father’s Name",
         "buyers-address": "Buyer’s Address",
         "vehicle-registration-number": "Vehicle Registration Number",
         "vehicle-chassis-number": "Vehicle Chassis Number",
         "vehicle-engine-number": "Vehicle Engine Number",
      ...
      Object.entries(formData).forEach(([key, rawVal]) => {
         ...
         // 1) Required check with field-specific label
         if (!val) {
            validationErrors[key] = `${labels[key] || key} is required`;
            return;
         }

         // 2) Field-specific additional checks
         switch (key) {
            case "agreement-year":
               if (!/^\d{4}$/.test(val)) {
                  validationErrors[key] = "Agreement year is Invalid";
               }
               break;

            case "sale-price":
               if (Number.isNaN(Number(val))) {
                  validationErrors[key] = "Sale Price must be a number";
               }
               break;

            // no extra checks for native date input
            default:
               break;
         }
      });
   ...
   const handleSubmit = async (e) => {
      ...
      const { isValid, errors: newErrors } = validateFormData(formData);
            ...
            <div className="agreement-form-wrapper">
               ...
               <form className="agreement-form" onSubmit={handleSubmit}>
                  <div className="form-inputs">
                     ...
                     <div>
                        <label htmlFor="vehicle-name-model">
                     ...
                     <div>
                        <label htmlFor="buyers-fathers-name">
                           Enter Buyer's Fathers Name:
                        </label>
                        <input
                     ...
                     <div>
                        <label htmlFor="vehicle-chassis-number">
                     ...
                     <div>
                        <label htmlFor="vehicle-engine-number">
